-- Sample workflow RPCs — logic ported 1:1 from the original MCSP
-- checkout_sample/return_sample/forward_sample/delete_sample/
-- set_sample_image/delete_buyer (schema.sql sections 5, 5c-5e, 9), only the
-- authorization checks swapped from is_super_admin()/current_hall_id() to
-- core.is_admin()/core.is_department_admin('mcsp')/mcsp.current_hall_id().
-- Schema-qualified as mcsp.* throughout; unqualified names would resolve
-- against `public` since these run as SECURITY DEFINER with a fixed
-- search_path.
...
begin
  ...
  from mcsp.movements m join mcsp.samples s on s.id = m.sample_id
...
begin
  ...
  if not (core.is_admin() or core.is_department_admin('mcsp')) and not mcsp.is_hall_manager_of(v_sample.hall_id) then
  ...
  if v_dest_hall_id is not null then
    update mcsp.samples set hall_id = v_dest_hall_id where id = v_sample.id;
  end if;
...
begin
  ...
  if not (core.is_admin() or core.is_department_admin('mcsp')) and not mcsp.owns_buyer(v_buyer_id) then
    raise exception 'Not authorized to update this sample';
  end if;

  update mcsp.samples set image_url = p_image_url where id = p_sample_id
...
grant execute on function mcsp.checkout_sample to authenticated;
grant execute on function mcsp.return_sample to authenticated;
grant execute on function mcsp.forward_sample to authenticated;
grant execute on function mcsp.clear_movement_history to authenticated;
grant execute on function mcsp.set_sample_image to authenticated;
grant execute on function mcsp.delete_sample to authenticated;
grant execute on function mcsp.delete_buyer to authenticated;
