...
import { listPanelMovements } from '../../../lib/mcsp/db'
import { exportPanelMovementsToExcel } from '../../../lib/mcsp/exportExcel'
import type { PanelMovementWithRelations } from '../../../lib/mcsp/dbTypes'
      ...
      <div className="border border-border rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={6} className="px-4 py-6 text-center text-text-secondary">Loading…</td>
              </tr>
            ) : movements.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-6 text-center text-text-secondary">No movements yet.</td>
              </tr>
            ) : (
              movements.map((m) => (
                <tr key={m.id} className="border-b border-border last:border-0">
                  <td className="px-4 py-2.5 text-text font-mono">{m.panel?.panel_code ?? '—'}</td>
